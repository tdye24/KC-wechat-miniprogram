    ...
    .then(result => {
      ...
      if (invalidTunnelIds.length) {
        ...
        invalidTunnelIds.forEach(tunnelId => {
          ...
          const index1 = zhengzhiList.indexOf(tunnelId)
          if (~index1) {
            zhengzhiList.splice(index1, 1)
          }
          const index2 = weiguanList.indexOf(tunnelId)
          if (~index2) {
            weiguanList.splice(index2, 1)
          }
          const index3 = hongguanList.indexOf(tunnelId)
          if (~index3) {
            hongguanList.splice(index3, 1)
          }
        })
      }
    })
...
function onConnect(tunnelId) {
  ...
  if (tunnelId in userMap) {
    ...
    var interval = setInterval(function() {
        ...
        if (num === 0 && connectedTunnelIds.length <= 1) {
          $broadcast('info', {
            status: "failure"
          })
          connectedTunnelIds.pop()
          clearInterval(interval)
          return
        } else if (connectedTunnelIds.length >= 2) {
          var length = connectedTunnelIds.length
          var userOneTunnelId = connectedTunnelIds[length - 1]
          var userTwoTunnelId = connectedTunnelIds[length - 2]

          $broadcast('info', {
            status: "success",
            userOne: JSON.stringify(userMap[userOneTunnelId]),
            userTwo: JSON.stringify(userMap[userTwoTunnelId]),
          })
          roomList.push({
            userOne: {
              tunnelId: userTwoTunnelId,
              num: 0,
              score: 0,
            },
            userTwo: {
    ...
    case 'speak':
      if (tunnelId in userMap) {
        $broadcast('speak', {
          ...
          'word': content.word,
          'userMap': userMap,
          'connectedTunnelIds': connectedTunnelIds
        })
      } else {
        $close(tunnelId)
      }
